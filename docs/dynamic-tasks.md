...
| `computeAndSaveValueWithLease`, `keepOutputs` | `Hyperion.Scheduler.Task.Task` | The same hook for keys of the `TaskKey` class, and the keep flag below. |
...
```sh
HYPERION_SCHEDULER_TEST=followups \
  stack exec -- hyperion-scheduler-test master -p '"shared"' -A '"yun124"'
```
