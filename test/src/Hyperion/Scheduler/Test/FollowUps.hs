...
import Hyperion.Scheduler.Test.LinearTransform (ProgramOptions (..),
                                                programOpts)
...
testJob :: Job Scheduler.Config -> OsPath -> FollowUpsProblem -> Job ()
...
runTestSlurm :: TestConfig.HPCName -> OsPath -> [FollowUpsProblem] -> IO ()
runTestSlurm hpcName baseDir problems = do
  ...
  let
    ...
    -- All scenarios run inside one 'hyperionMain', one SLURM job each. The
    -- same executable serves as the worker, and its 'main' must not reach
    -- 'hyperionMain' more than once: the worker branch returns when its job
    -- is done, and a second call would start a second worker against a
    -- master that has already finished.
    clusterComputation options = forM_ problems $ \problem -> do
      let
        jobType = MPIJob 1 problem.nodeCpus
        workDir = baseDir </> "mpi_1_" <> showOs problem.nodeCpus
      _ <- local (setJobTime (20 * minute) . setJobType jobType . setJobOptions options.sbatchOptions) $
...
runTest :: IO ()
runTest = do
  baseDir <- makeAbsolute "tmp/hyperion-scheduler-followups-test"
  let
    problems =
  ...
  runTestSlurm "expanse" baseDir problems
